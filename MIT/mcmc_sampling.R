...
source('~/loglikelihood_sampling.R')
...
raw_features = read.csv('~/manu_features.csv',stringsAsFactors = F)
...
input_S = readline('Enter the index of the sampling fraction column: ')#18
input_Y = readline('Enter the index of the refusal fraction column: ')#20
input_Nshipments = readline('Enter the index of the number of shipments column: ')#3
input_Nsampled = readline('Enter the index of the number of sampled shipments column: ')#17
...
Nshipments=features[,as.numeric(input_Nshipments)]
Nsampled=features[,as.numeric(input_Nsampled)]
...
S_S = S[S>0]
Y_S = Y[S>0]
Nshipments_S=Nshipments[S>0]
Nsampled_S=Nsampled[S>0]
...
for (i in 2:n){
  ...
  l_old = loglikelihood_sampling(X,S,Y,Nshipments,Nsampled,Beta_old,Gamma_old,sigma_old,hyperpriors[1],rho_old)[1]
  l_new = loglikelihood_sampling(X,S,Y,Nshipments,Nsampled,Beta_old,Gamma_old,sigma_new,hyperpriors[1],rho_old)[1]
  ...
  for (f in 1:nfeatures){
    ...
    l_old = loglikelihood_sampling(X,S,Y,Nshipments,Nsampled,Beta_old,Gamma_old,sigma_old,hyperpriors[1],rho_old)[1]
    l_new = loglikelihood_sampling(X,S,Y,Nshipments,Nsampled,Beta_old,Gamma_new,sigma_old,hyperpriors[1],rho_old)[1]
...
for (i in 2:n){
  ...
  l_old = loglikelihood_sampling(X_S,S_S,Y_S,Nshipments_S,Nsampled_S,Beta_old,Gamma_old,sigma_old,hyperpriors[1],rho_old)[2]
  l_new = loglikelihood_sampling(X_S,S_S,Y_S,Nshipments_S,Nsampled_S,Beta_old,Gamma_old,sigma_old,hyperpriors[1],rho_new)[2]
  ...
  for (f in 1:nfeatures){
    ...
    l_old = loglikelihood_sampling(X_S,S_S,Y_S,Nshipments_S,Nsampled_S,Beta_old,Gamma_old,sigma_old,hyperpriors[1],rho_old)[2]
    l_new = loglikelihood_sampling(X_S,S_S,Y_S,Nshipments_S,Nsampled_S,Beta_new,Gamma_old,sigma_old,hyperpriors[1],rho_old)[2]
...
Mat = data.frame(X,Nshipments,Nsampled,S,Y)
set.seed(123)
split = sample.split(S,0.5)
train = subset(Mat,split=TRUE)
test = subset(Mat,split=FALSE)
X = train[,1:(ncol(Mat)-4)]
S = train[,(ncol(Mat)-1)]
Y = train[,ncol(Mat)]
Nshipments = train[,(ncol(Mat)-3)]
Nsampled = train[,(ncol(Mat)-2)]
nfeatures = ncol(X)
ndata = nrow(X)
...
# Sampling out-of-sample prediction

X = test[,1:(ncol(Mat)-4)]
S = test[,(ncol(Mat)-1)]
Nshipments = test[,(ncol(Mat)-3)]
Nsampled = test[,(ncol(Mat)-2)]
...
# sampling out of sample spearman correlation
index=S>0
cor_q = cor(qmean[index],S[index],method="s")
cor_test_q = suppressWarnings(cor.test(qmean[index],S[index],method="s",alternative="greater"))

#refusal out-of-sample prediction
...
# refusal out of sample spearman correlation
index=Y>0
cor_p = cor(pmean[index],Y[index],method="s")
cor_test_p = suppressWarnings(cor.test(pmean[index],Y[index],method="s",alternative="greater"))
...
write.csv(q_significant_features,'significant features for sampling.csv')
write.csv(p_significant_features,'significant features for refusal.csv')
features_table=features;
features_table$inspection_score=q_mean;
features_table$risk_score=p_mean;
write.csv(features_table,'sampling_features_scores.csv')
